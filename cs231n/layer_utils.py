...
def affine_bn_relu_forward(x, w, b, param):
    """
    Convenience layer that perorms an affine transform followed by a ReLU
...
def affine_ln_relu_forward(x, w, b, param):
    """
    Convenience layer that perorms an affine transform followed by a ReLU
    ...
    a, fc_cache = affine_forward(x, w, b)
    a_norm, bn_cache = layernorm_forward(a, gamma, beta, bn_param)
    out, relu_cache = relu_forward(a_norm)
    cache = (fc_cache, bn_cache, relu_cache)
    return out, cache
...
def affine_ln_relu_backward(dout, cache):
    ...
    fc_cache, bn_cache, relu_cache = cache
    da = relu_backward(dout, relu_cache)
    da_norm, dgamma, dbeta = layernorm_backward(da, bn_cache)
    dx, dw, db = affine_backward(da_norm, fc_cache)
    return dx, dw, db, dgamma, dbeta
